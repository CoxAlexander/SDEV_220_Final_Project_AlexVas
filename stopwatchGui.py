...
class gui:
    ...
    def lap(self):
        if not(gui.stop or gui.pause):
            ...
            self.lapTimer = sw.lap(startTime=time.time())
            self.lapNum +=1
            self.lapList.append(tk.Label(self.root, font=('Helvetica' ,20), text=f'lap {self.lapNum}: {self.lapTimer.lapFunc(self.timer.currentTime(time.time()))}'))
            ...
            self.root.update()
            self.timer.storeData(pT=self.timer.getPassedTime(), sT=self.timer.getStartTime())
            self.lapTimer.storeLapData(lN=self.lapNum,lT=self.lapTimer.getLapTime())
